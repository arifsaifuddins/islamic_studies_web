...
function AdminField({ url }) {
  ...
  return (
    <>
      ...
      <input required type="text" placeholder="اسم المشرف" onChange={(e) => setName(e.target.value)} className="named mt-4 bg-transparent py-1 pr-3 rounded-lg  border outline-none border-yellow-600 w-[100%]" />
      <input required type="email" placeholder="بريد أو مسمى المشرف" onChange={(e) => setEmail(e.target.value)} className="mailed mt-2 bg-transparent py-1 pr-3 rounded-lg  border outline-none border-yellow-600 w-[100%]" />
      <input required type="text" placeholder="كلمة المرور" onChange={(e) => setPass(e.target.value)} className="passed mt-2 bg-transparent py-1 pr-3 rounded-lg  border outline-none border-yellow-600 w-[100%]" />
      <input required type="password" placeholder="تأكيد كلمة المرور" onKeyUp={(e) => e.which === 13 && submitAdmin()} onChange={(e) => setConf(e.target.value)} className="confed mt-2 bg-transparent py-1 pr-3 rounded-lg  border outline-none border-yellow-600 w-[100%]" />
      {
