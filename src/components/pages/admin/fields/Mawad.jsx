...
function MawadField({ url }) {
  ...
  return (
    <>
      ...
      <label htmlFor="categor" className="my-2 block font-bold text-blue-600">البرنامج :</label>
      <select required id="categor" onChange={(e) => setProgram(e.target.value)} className="progm bg-transparent p-3  rounded-lg  border outline-none border-blue-600 w-[100%]" >
      ...
      <label htmlFor="categori" className="my-2 block font-bold text-blue-600">الفصل الدراسي :</label>
      <select required id="categori" onChange={(e) => setClass(e.target.value)} className="classes bg-transparent p-3  rounded-lg  border outline-none border-blue-600 w-[100%]" >
      ...
      <label htmlFor="madah" className="my-2 block font-bold text-blue-600">المادة :</label>
      <input type="text" required id="madah" onChange={(e) => setMadah(e.target.value)} placeholder="المادة" className="madah bg-transparent py-1 p-3  rounded-lg  border outline-none border-blue-600 w-[100%] mb-2" />
      <label htmlFor="time" className="my-2 block font-bold text-blue-600">الساعة :</label>
      <input type="number" required id="time" onChange={(e) => setTime(e.target.value)} placeholder="الساعة " className="time bg-transparent py-1 p-3  rounded-lg  border outline-none border-blue-600 w-[100%] mb-2" />
      {
        (Commited == false) ? <p className="bmiss cursor-not-allowed text-center mt-4 bg-blue-800 text-slate-400 py-1 rounded-lg font-bold w-[100%]">إضافة المواد الدراسية</p> : <p className="cursor-pointer text-center mt-4 bg-blue-600 text-white py-1 rounded-lg hover:bg-blue-700 font-bold w-[100%]" onClick={() => submitStudies()}>إضافة المواد الدراسية</p>
      }
    </>
  )
}
