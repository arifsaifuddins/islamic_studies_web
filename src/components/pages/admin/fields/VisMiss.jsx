...
function VisMissField({ url }) {
  ...
  return (
    <>
      ...
      <label htmlFor="category" className="my-2 block font-bold text-blue-600">النوع :</label>
      <select required id="category" onChange={(e) => setCategory(e.target.value)} className="viss bg-transparent p-3  rounded-lg  border outline-none border-blue-600 w-[100%]" >
      ...
      <div className="misi">
        ...
        <textarea required id="mission" onChange={(e) => setPoint(e.target.value)} placeholder='الرؤية أو الهدف أو القيمة' className="mis misis bg-transparent py-1 pr-3 rounded-lg  border outline-none border-blue-600 w-[100%]" />
      </div>
      {
        (Commited == false) ? <p className="bmiss cursor-not-allowed text-center mt-4 bg-blue-800 text-slate-400 py-1 rounded-lg font-bold w-[100%]">إضافة الرؤيات أو الأهداف</p> : <p className="cursor-pointer text-center mt-4 bg-blue-600 text-white py-1 rounded-lg hover:bg-blue-700 font-bold w-[100%]" onClick={() => submitVisMiss()}>إضافة الرؤيات أو الأهداف</p>
      }
    </>
  )
}
