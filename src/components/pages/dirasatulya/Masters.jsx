import React from 'react'
import { Helmet } from 'react-helmet'

const Masters = () => {
  return (
    <>
      <Helmet>
        <title>Islamic Studies | الماجستير</title>
      </Helmet>
      <div className='md:h-80 h-72 flex bg-stone-700 bg-opacity-50 text-white'>
        <h1 className='lg:text-5xl text-4xl text-center m-auto'>الماجستير</h1>
      </div>
      <div className="bg-white border-t-4 border-yellow-600">
        <div className="mx-auto xl:w-[1200px] w-full px-4 xl:px-0 relative">
          <h2 className='px-3 py-1 absolute xl:right-0 right-4 rounded-bl-lg bg-yellow-600 w-max text-white'>ماجستير السنة وعلوم الحديث</h2>
          <div className="mx-auto lg:w-[900px] w-full text-end leading-9 py-32">
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الأول</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الثاني</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-slate-100 border-t-4 border-yellow-600">
        <div className="mx-auto xl:w-[1200px] w-full px-4 xl:px-0 relative">
          <h2 className='px-3 py-1 absolute xl:left-0 left-4 rounded-br-lg bg-yellow-600 w-max text-white'>ماجستير الدعوة والسيرة</h2>
          <div className="mx-auto lg:w-[900px] w-full text-end leading-9 py-32">
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الأول</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الثاني</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white border-t-4 border-yellow-600">
        <div className="mx-auto xl:w-[1200px] w-full px-4 xl:px-0 relative">
          <h2 className='px-3 py-1 absolute xl:right-0 right-4 rounded-bl-lg bg-yellow-600 w-max text-white'>ماجستير العقيدة والفكر الإسلامي</h2>
          <div className="mx-auto lg:w-[900px] w-full text-end leading-9 py-32">
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الأول</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
            <div className="mb-20">
              <h2 className='text-lg text-white w-full px-4 py-2 bg-yellow-600 text-center'>الفصل الدراسي الثاني</h2>
              <ul className='bg-white shadow'>
                <li className='flex flex-row-reverse items-center font-bold gap-6 border-b-4 px-6 py-2 text-yellow-600'>
                  <p className='w-[65%] text-center'>المواد الدراسية</p>
                  <div className='border-2 h-6 border-yellow-600' />
                  <p className='w-[30%] text-center'>الساعات</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
                <li className='flex flex-row-reverse items-center gap-6 border-b-4 px-6 py-2'>
                  <p className='w-[65%]'>الفقه الإسلامي</p>
                  <div className='border-2 h-6 border-gray-600' />
                  <p className='w-[30%] text-center'>2</p>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </>
  )
}

export default Masters
