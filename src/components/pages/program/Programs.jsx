    ...
    <>
      ...
      <div className="bg-white border-t-4 border-blue-600">
        <div className="mx-auto xl:w-[1200px] w-full px-4 xl:px-0 relative">
          <h2 className='px-3 py-1 absolute rounded-bl-lg bg-blue-600 w-max text-white'>البرامج والأخبار والمؤتمرات</h2>
          <div className="mx-auto lg:w-[900px] w-full  leading-9 py-32">
            <div className="flex-row flex items-center gap-4 relative">
              <input onChange={(e) => e.target.value == '' ? nav('/programs') : nav(`/programs/s/${e.target.value}`)} type="text" className='bg-transparent text-base py-2 outline-none shadow-none  w-full pl-4 pr-12 border rounded-lg border-blue-600' placeholder='ابحث عن برنامج' />
              <div className="p-2 text-2xl rounded-lg text-blue-600 absolute right-2">
                <FiSearch />
              </div>
              <div className="p-2 cursor-pointer text-2xl relative rounded-lg bg-blue-600 text-white" onClick={() => { document.querySelector('.filter').classList.toggle('hidden'); document.querySelector('.filter').classList.toggle('flex') }}>
                <FiSliders />
            ...
            <p className="my-8 text-center text-lg font-bold text-blue-600">جميع البرامج والمؤتمرات والأخبار</p>
            <Outlet />
          </div>
        </div>
      </div>
